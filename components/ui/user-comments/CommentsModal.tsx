...
export default function CommentsModal({ driver, myPositive, myNegative, onClose, onSave }: CommentsModalProps) {
    ...
    return (
        <div
            className="fixed inset-0 z-500 flex items-center justify-center  backdrop-blur-sm"
            onClick={(e) => { if (e.target === e.currentTarget) handleClose() }}
        >
            <div className="bg-card-bg border border-card-border rounded-xl w-150 overflow-hidden" style={{ transform: "translateY(0) scale(1)" }}>


                <div className="flex items-center justify-between px-6 py-4.5 border-b border-card-border sticky top-0 bg-[#111115] z-10">
                    ...
                    <button
                        onClick={handleClose}
                        className="w-8 h-8 rounded-md bg-[#18181e] border border-white/11 text-[#7a7870] flex items-center justify-center hover:bg-[#202028] hover:text-[#eeeae2] transition-all shrink-0 cursor-pointer"
                    >
                ...
                <div className="flex justify-end px-6 py-4 border-t border-white/6 bg-[#18181e] rounded-b-xl">
                    <button
                        onClick={handleClose}
                        className="px-5 py-2 rounded-md bg-[#202028] border border-white/18 text-[13px] font-semibold text-[#eeeae2] hover:bg-[#26262f] transition-all cursor-pointer"
                    >
                        Done
                    </button>
                </div>
            ...
            {toast && (
                <div className={`fixed bottom-6 right-6 z-999 flex items-center gap-2 px-4.5 py-2.75 rounded-lg bg-[#18181e] border text-[13px] font-medium max-w-75 pointer-events-none shadow-lg
                    ${toast.type === "pos" ? "border-l-[3px] border-l-[#22c069] border-white/18 text-[#eeeae2]" : ""}
                    ${toast.type === "neg" ? "border-l-[3px] border-l-[#e84040] border-white/18 text-[#eeeae2]" : ""}
                    ${toast.type === "info" ? "border-l-[3px] border-l-[#e8410a] border-white/18 text-[#eeeae2]" : ""}
                `}>
                    {toast.type === "pos" && <svg className="w-3.5 h-3.5 text-[#22c069] shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M3 8l4 4 6-7" /></svg>}
                    {toast.type === "neg" && <svg className="w-3.5 h-3.5 text-[#e84040] shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="8" cy="8" r="6" /><path d="M8 5v3.5" /><circle cx="8" cy="11.5" r=".5" fill="currentColor" /></svg>}
                    {toast.type === "info" && <svg className="w-3.5 h-3.5 text-[#e8410a] shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="8" cy="8" r="6" /><path d="M8 7v4" /><circle cx="8" cy="5" r=".5" fill="currentColor" /></svg>}
                    {toast.msg}
                </div>
            )}
        </div>
    ...
    return (
        <div className="flex flex-col gap-2.5">
            ...
            {comments.map((c, i) => (
                <div
                    key={c.id}
                    className={`flex gap-2.5 items-start bg-[#18181e] border rounded-[7px] px-3 py-2.5 transition-colors ${editingId === c.id ? "border-[#e8410a]" : "border-white/11"}`}
                >
                    <span className="text-xs text-text-muted shrink-0 pt-px w-4 text-right">{i + 1}</span>
                        ...
                        {editingId === c.id ? (
                            <>
                                ...
                                <div className="flex items-center justify-between mt-1.5">
                                    ...
                                    <div className="flex gap-1.5">
                                        <button onClick={() => onSave(c)} className="flex items-center gap-1 px-2.5 py-1 rounded-sm bg-[#e8410a] text-white text-[11px] font-medium border border-[#e8410a] hover:bg-[#c93508] transition-colors cursor-pointer">Save</button>
                                        <button onClick={onCancelEdit} className="flex items-center gap-1 px-2.5 py-1 rounded-sm border border-white/11 text-[#7a7870] text-[11px] font-medium hover:text-[#eeeae2] hover:border-white/18 transition-colors cursor-pointer">Cancel</button>
                                        <button onClick={() => onDelete(c)} className="flex items-center gap-1 px-2.5 py-1 rounded-sm border border-[rgba(232,64,64,0.22)] text-[#e84040] text-[11px] font-medium hover:border-[#e84040] transition-colors cursor-pointer">
                            ...
                            <>
                                ...
                                <div className="flex items-center gap-1.5 mt-1.5">
                                    <button onClick={() => onEdit(c)} className="flex items-center gap-1 px-2.5 py-1 rounded-sm border border-white/11 text-[#7a7870] text-[11px] font-medium hover:text-[#eeeae2] hover:border-white/18 transition-colors cursor-pointer">
                                        <Pencil className="w-2.75 h-2.75" />
                                        Edit
                                    </button>
                                    <button onClick={() => onDelete(c)} className="flex items-center gap-1 px-2.5 py-1 rounded-sm border border-[rgba(232,64,64,0.22)] text-[#e84040] text-[11px] font-medium hover:border-[#e84040] transition-colors cursor-pointer">
                                        <X className="w-2.75 h-2.75" />
            ...
            {/* Empty/new slot */}
            {canAdd && (
                <div className="flex gap-2.5 items-center bg-[#18181e] border border-dashed border-white/11 rounded-[7px] px-3 py-2.5">
                    <span className="text-xs text-text-muted shrink-0 w-4 text-right">{comments.length + 1}</span>
                    <div className="flex-1 flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.25 rounded-sm text-[11px] font-semibold border transition-all disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed
                                    ${isPos
                            ...
                            >
                                <svg className="w-2.75 h-2.75" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2">
